...
%% domain
Lx= (3/4) * 0.0254;                  %  width (m)
Ly= 14 * 0.0254;                  %  depth (m)
Lz= 14 * 0.0254;                  %  height (m)
...
T_0= 40;                 % Initial temperature in all nodes 
T_right  = T_0;        % temperature at x=Lx
T_left   = T_0;        % temperature at x=0 
T_top  = T_0;             % temperature at z=Lz 
T_bott  = T_0;            % temperature at z=0  
T_front= T_0;            % temperature at y=0 
T_back= T_0;            % temperature at y=Ly
...
if mod(iter,2)==0  % e.g. plot every 5 iter
    plot3D_conduction(X,Y,Z,Lx,Ly,Lz,dx,dy,dz,T,T_min,T_max,isovalues,iter,t); 
end
if video >0 
         frame = getframe(gcf);
